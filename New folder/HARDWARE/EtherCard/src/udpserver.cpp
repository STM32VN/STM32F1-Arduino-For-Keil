...
void EtherCard::udpServerListenOnPort(UdpServerCallback callback, uint16_t port) {
    ...
    {
        //listeners[numListeners] = (UdpServerListener) {callback, port, true};//thay the nhu SYS
				listeners[numListeners].callback = callback;
				listeners[numListeners].port = port;
				listeners[numListeners].listening = true;
        numListeners++;
    }
}
