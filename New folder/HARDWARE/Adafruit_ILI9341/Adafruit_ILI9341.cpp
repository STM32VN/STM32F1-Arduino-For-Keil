...
void Adafruit_ILI9341::spiwrite(uint8_t c) {
  ...
  if (hwSPI) {
		SPIx.write(c);// Faster than transfer as we don't need to read
//#if defined (__AVR__)
...
//    SPIx.setClockDivider(11); // 8-ish MHz (full! speed!)
...
void Adafruit_ILI9341::writecommand(uint8_t c) {
  ...
  digitalWrite(_dc, LOW);//*dcport &=  ~dcpinmask;  
  SPIx.write(c);
  digitalWrite(_dc, HIGH);////*csport |= cspinmask;
}


void Adafruit_ILI9341::writedata(uint8_t c) {  
  //digitalWrite(_cs, LOW);//*csport &= ~cspinmask;
	//digitalWrite(_dc, HIGH);//*dcport |=  dcpinmask;
  SPIx.write(c);
  //digitalWrite(_cs, HIGH);//*csport |= cspinmask;
} 
...
static inline void spi_begin(void) {
  SPIx.beginTransaction(SPISettings(84000000, MSBFIRST, SPI_MODE0,SPI_DATASIZE_8BIT));
}
...
void Adafruit_ILI9341::commandList(uint8_t *addr) {
  ...
  while(numCommands--) {                 // For each command...
    ...
    while(numArgs--) {                   //   For each argument...
        SPIx.write(pgm_read_byte(addr++));  //     Read, issue argument
    }
...
void Adafruit_ILI9341::setAddrWindow(uint16_t x0, uint16_t y0, uint16_t x1,
 uint16_t y1) {
  ...
  //digitalWrite(_cs, LOW);//*csport &= ~cspinmask;
  SPIx.write(x0 >> 8);
  SPIx.write(x0 & 0xFF);     // XSTART 
  SPIx.write(x1 >> 8);
  SPIx.write(x1 & 0xFF);     // XEND

  writecommand(ILI9341_PASET); // Row addr set
  //digitalWrite(_dc, HIGH);//*dcport |=  dcpinmask;
  //digitalWrite(_cs, LOW);//*csport &= ~cspinmask;
  SPIx.write(y0>>8);
  SPIx.write(y0);     // YSTART
  SPIx.write(y1>>8);
  SPIx.write(y1);     // YEND
...
//   digitalWrite(_cs, LOW);
//   SPIx.write(0xD9);  // woo sekret command?
//   digitalWrite(_dc, HIGH); // data
//   SPIx.write(0x10 + index);
//   digitalWrite(_cs, HIGH);
...
//   digitalWrite(_cs, HIGH);
	...
	digitalWrite(_dc, LOW);//dc_command();
  digitalWrite(_cs, LOW);//cs_clear();
  SPIx.transfer(c);
  digitalWrite(_dc, HIGH);//dc_data();
