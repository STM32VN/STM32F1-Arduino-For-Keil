...
class Adafruit_ILI9341_DMA : public Adafruit_GFX_AS {

 public:

  Adafruit_ILI9341_DMA(int8_t _CS, int8_t _DC, int8_t _RST = -1);
...
#define DMA_ON_LIMIT 230 // do DMA only for more data than this
#define SAFE_FREQ  24000000ul // 24MHz for reading

#define writePixel drawPixel

#define dc_command() ( digitalWrite(_dc, LOW) ) // 0
#define dc_data()    ( digitalWrite(_dc, HIGH) )     // 1
#define cs_clear()   ( digitalWrite(_cs, LOW) )
...
//#define miso_in()    ( (*misoport)&misopinmask )
  ...
  inline uint8_t spiread(void)  { return SPIx.transfer(0x00); }
  inline uint8_t readdata(void) { return SPIx.transfer(0x00); }
  inline void    writedata(uint8_t c)   { SPIx.write(c); }
  inline void    spiwrite(uint16_t c)   { SPIx.write16(c); }		//16 bit
  inline void    spiwrite16(uint16_t c) { SPIx.send16(c); } // 8 bit mode
