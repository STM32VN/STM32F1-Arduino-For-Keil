...
// Constructor when using software SPIx.  All output pins are configurable.
...
// Constructor when using hardware SPIx.  Faster, but must use SPI pins
...
void Adafruit_ST7735::writecommand(uint8_t c) {
  ...
  if (hwSPI) SPIx.beginTransaction(spisettings);
#endif
...
void Adafruit_ST7735::commonInit(const uint8_t *cmdList) {
  ...
  if(hwSPI) { // Using hardware SPI
    ...
    SPIx.setClockDivider(24); // 4 MHz (half speed)
#else
    SPIx.setClockDivider(SPI_CLOCK_DIV4); // 4 MHz (half speed)
#endif
