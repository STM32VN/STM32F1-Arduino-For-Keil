...
void TCS3200::begin()
{
	...
	attachInterrupt(PA7, ISR_INTO, RISING);//RISING//CHANGE
	saveMillis = millis() + 1;
...
void TCS3200::getRGBrelativClearWithCorrection()
{
	...
	if (currentColor == RED) {
		temp = (((uint32_t)summRGBC[currentColor]) << (8 + 3)) / (5 * divider);  // *8/5 R
	}

	if (currentColor == GREEN) {
		temp = ((((uint32_t)summRGBC[currentColor]) << 8) * 5) / (divider * 3);  //*5/3 G 
	}

	if (currentColor == BLUE) {
		temp = ((((uint32_t)summRGBC[currentColor]) << (8 - 1)) * 3) / divider;  //*3/2 B  
	}

	if (currentColor != CLEAR) {
		if (temp > 255) temp = 255;
		RGBvalue[currentColor - 1] = (uint8_t)temp;    // RGB.
	}
}
